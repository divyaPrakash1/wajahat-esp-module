...
import { environment, ENV_BUILD } from "src/environments/environment";
// import { ApiError } from "src/app/shared/common/api-error";
...
// import { APP_APPEARANCE_ADDON_NAME } from "./app-appearance.service";
...
export class AuthService {
  ...
  constructor(private http: HttpClient, private router: Router, 
    // private _exdrLanguageService: ExdrLanguageService
    ) {
    this.espOptions = {
  ...
  get idenediTokenExpiresAt(): number {
    if (!this._expiresAt) {
      this._expiresAt = +localStorage.getItem(TOKEN_EXPIRES_AT_KEY) ?? 0;
    }
    ...
    return this.http.get<any>(url, options).pipe(
      map(addons => {
        ...
        if (powerBiAddon) {
          ...
          powerBiAuthItem.PublicProperties.powerBiData = JSON.parse(powerBiAddon.PublicProperties.powerBiData) ?? null;
      ...
      tap(() => {
        ...
        this._groupWiseAddon$[groupId] = undefined;
      })
    );
  }
  ...
  initTabOrderAddon(tabOrderAddon: Addon): Addon {
    let parsed;
    if(this.isJSON(tabOrderAddon && tabOrderAddon.PublicProperties && tabOrderAddon.PublicProperties.jsonData)) {
      parsed = JSON.parse(tabOrderAddon.PublicProperties.jsonData);
    } else {
      parsed = tabOrderAddon?.PublicProperties?.jsonData;
    }
