...
import { CustomFieldModel, CustomFieldLookupModel } from '../../shared/models/CustomFieldModel-activity';
...
import { TranslateToLocalePipe } from '../pipes/translate-to-locale-activity.pipe';
import { ActionValue } from '../../shared/models/action-model-activity';
import * as moment from 'moment';
import { CurrencyModel } from '../../shared/models/currency-model-activity';
import { TranslateService } from '@ngx-translate/core';
@Injectable({
  providedIn: 'root'
})
export class CustomFieldService {
  constructor(private _datePipe: DatePipe, private translateToLocalePipe: TranslateToLocalePipe, private _translateService: TranslateService) { }
  ...
  getFeildValue(feild: CustomFieldModel) {
    switch (feild.type) {
      ...
      case FeildType.DateTime: case FeildType.Date: {
        if (feild.value) {
          let momentDate = moment.parseZone(feild.value);
    ...
    switch (field.type) {
      ...
      case FeildType.DateTime: {
        if (field.value) {
          ...
          if (date.isValid()) {
            value = moment(field.value).utc(true).format('YYYY-MM-DDT00:00:00Z');//'YYYY-MM-DDT00:00:00Z'
          } else {
            value = field.value;
          }
        }
