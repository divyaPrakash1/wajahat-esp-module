...
import { environment, ENV_BUILD } from "../../../../environments/environment";
import { Addon } from "../../shared/models/addon.model-activity";
import { APP_APPEARANCE_ADDON_NAME } from "./app-appearance-activity.service";
import { CentralizedActivity } from "../../activity/enums";
import { ApiError } from "../common/api-error-activity";
...
export const IDENEDI_CODE_STORAGE_KEY = "idenedi.auth.code";
...
export class ActivityAuthService {
  ...
  private _idenediToken: string;
  private _groupWiseAddon$: { [groupId: string]: Subject<Addon[]> } = {};
  ...
  get idenediGroupId(): string {
    if (
      environment.enableGroupsSelection &&
  ...
  ensureIdenediToken(): Observable<string> {
    if (!this.isLoggedIn()) {
      return of(null);
    }
  ...
  login(
    ...
    return this.http
      .post(`${environment.idenediProviderUrl}/api/account/Login`, body)
      .pipe(
  ...
  register(
    ...
    return this.http
      .post(`${environment.idenediProviderUrl}/api/account/Register`, body)
      .pipe(
        map((response: any) => response.Token),
        tap(token => (this.idenediToken = token))
      );
  ...
  logout(redirectToLogin: boolean = true): void {
    this.idenediToken = null;
    localStorage.removeItem(IDENEDI_USER_ID);
    localStorage.removeItem("higherUser");
    localStorage.removeItem(ESP_LOGIN_DATA);
    localStorage.removeItem(IDENEDI_CODE_STORAGE_KEY);
    localStorage.removeItem(CentralizedActivity.SourceSystemId);
    localStorage.removeItem(CentralizedActivity.SourceTenantId);
    localStorage.removeItem(CentralizedActivity.SourceObjectTypeId);
    localStorage.removeItem(CentralizedActivity.SourceObjectId);
    this._addonsData = {};
    this._groupWiseAddon$ = {};
    this.removeEspToken();
    if (environment.enableGroupsSelection) {
      this.idenediGroupId = null;
    }
    if (redirectToLogin) {
  ...
  getAddonsAuth(groupId: string, changeLanguage = true): Observable<Addon[]> {
    ...
    return this.http.get<any>(url, options).pipe(
      map(addons => {
            const simpleStrataAddons = addons.find(
          a => a.Type.toLowerCase() === AuthType.SimpleStrata
        );
        if(simpleStrataAddons && simpleStrataAddons.PublicProperties) {
          localStorage.setItem("organziationId", simpleStrataAddons.PublicProperties.organziationId)
        }
        if (!this._addonsData[groupId]) {
          this._addonsData[groupId] = [];
        }
        const technadoptAddon = addons.find(
          a => a.Type.toLowerCase() === AuthType.Technadopt
        );
        if (technadoptAddon) {
          this._addonsData[groupId].push({
            ...
            technadoptAuthData: null
          });
        }
        const espAddon = addons.find(
          a => a.Type.toLowerCase() === AuthType.ESP
        );
        if (espAddon) {
          ...
          localStorage.setItem("espOrganziationId", orgId);
          if (orgId) {
            espAuthItem.OrganziationId = +orgId;
          }
          espAuthItem.Configuration = espAddon.Configuration;
          const claimOwnershipData =
            espAddon.PublicProperties["claimOwnershipData"];
        ...
        const customTabAddon: Addon[] = addons.filter(
          a => a.Type.toLowerCase() === AuthType.CustomTab
        );
        if (customTabAddon) {
          customTabAddon.forEach(element => {
            this._addonsData[groupId].push({ ...element });
          });
        }

        const higherAddon = addons.find(
          a => a.Type.toLowerCase() === AuthType.Higher
        );
        if (higherAddon) {
        ...
        const powerBiAddon = addons.find(
          a => a.Type.toLowerCase() === AuthType.PowerBi
        );
        if (powerBiAddon) {
          ...
          powerBiAuthItem.PublicProperties.powerBiData = JSON.parse(
            powerBiAddon.PublicProperties.powerBiData
          );
        ...
        const intajyAddon: Addon = addons.find(
          a => a.Type.toLowerCase() === AuthType.Intajy
        );
        if (intajyAddon) {
        ...
        const appAppearanceAddon: Addon = addons.find(
          a => a.Type === AuthType.AppAppearance
        );
        if (appAppearanceAddon) {
          this._addonsData[groupId].push({ ...appAppearanceAddon });
        }

        const engagementPro: Addon = addons.find(
          a => a.Type === AuthType.EngagementPro
        );
        if (engagementPro) {
          this._addonsData[groupId].push({ ...engagementPro });
        }
        if(changeLanguage){
          this.changeLanguage(groupId);
        }
  ...
  changeLanguage(groupId: string){
    // if(environment.build == ENV_BUILD.STEMEXE && environment.staging){
    //   const appearanceAddon = this._addonsData[groupId].find(
    //     a => a.Type === APP_APPEARANCE_ADDON_NAME
    //   );
    //   if(appearanceAddon && appearanceAddon.PublicProperties && appearanceAddon.PublicProperties.language){
    //     const langArray: string[] = appearanceAddon.PublicProperties.language.split(',');
    //     return this._exdrLanguageService.initializeLangulage(langArray);
    //   }
    // }
    return null;
  }
  ...
  createNewAddon(addon: Addon, groupId: string, type: AuthType) {
    ...
    return this.http
      .post<any>(
        `${environment.apiHost}/api/group/${groupId}/addon?type=${type}`,
        data,
        options
      )
      .pipe(
  ...
  updateAddon(addon: Addon, groupId: string) {
    const authHeaders = this.buildAuthHeader(AuthType.Idenedi);
    const options = {
      headers: authHeaders.append("Idenedi-Api-Version", "4")
    };
    ...
    return this.http
      .put<any>(
        `${environment.apiHost}/api/group/${groupId}/addon/${addon.InstanceId}`,
        data,
        options
      )
      .pipe(
  ...
  deleteAddon(groupId: string, addonId: string) {
    ...
    return this.http
      .delete<any>(
        `${environment.apiHost}/api/group/${groupId}/addon/${addonId}`,
        options
      )
      .pipe(
        tap(() => {
          // Remove all and fetch again
          this._addonsData[groupId] = this._addonsData[groupId].filter(
  ...
  initTabOrderAddon(tabOrderAddon: Addon): Addon {
    let parsed = JSON.parse(tabOrderAddon.PublicProperties.jsonData);
    if (Array.isArray(parsed.data)) {
      tabOrderAddon.PublicProperties.jsonData = parsed;
      return { ...tabOrderAddon };
    }
    parsed = JSON.parse(parsed.data);
    tabOrderAddon.PublicProperties.jsonData = {
      data: parsed
    };
    return { ...tabOrderAddon };
  }
  ...
  ensureTechnadoptAuth(): Observable<Addon> {
    return this.getAddonsAuth(this.idenediGroupId).pipe(
      switchMap(addons => {
        ...
        // we only need to that token when user goes to Products page. For that we have Productresolver
        if (technadoptAddon?.technadoptAuthData) {
          if (
            this.isTechnadoptTokenExpired(
              technadoptAddon.technadoptAuthData.expiresIn
            )
          ) {
            // Relogin
            technadoptAddon.technadoptAuthData = null;
  ...
  getTechnadoptAuthorizedTokenIfRequired(
    technadoptAddon: Addon
  ): Observable<Addon> {
    if (
      technadoptAddon &&
      (!technadoptAddon.technadoptAuthData ||
        this.isTechnadoptTokenExpired(
          technadoptAddon.technadoptAuthData.expiresIn
        ))
    ) {
  ...
  getLegacyTechnadoptAuth(): Observable<Addon> {
    ...
    return this.http
      .post<any>(`${TECHNADOPT_AUTH_SETTINGS.domain}/token`, body.toString(), {
        headers
      })
      .pipe(
        map(({ access_token }) => {
  ...
  ensureEspToken(): Observable<Addon> {
    return this.getAddonsAuth(this.idenediGroupId).pipe(
      switchMap(addons => {
        const espAddon = addons.find(
  ...
  ensureSimpleStrataToken(): Observable<Addon> {
  ...
  buildAuthHeader(authType: AuthType, engProUserId?: string): HttpHeaders {
    let headers;
  ...
  getAuthAddonByType(authType: AuthType): Observable<Addon> {
    switch (authType) {
  ...
  getEspToken(groupId: string) {
    if (this._addonsData[groupId]?.length) {
      const addon = this._addonsData[groupId].find(x => x.Type == AuthType.ESP);
      if (addon) {
  ...
  // getTechnadoptTokenByCredentials(
  //   username: string,
  //   password: string
  // ): Observable<any> {
  //   const body = new HttpParams({
  ...
  private removeEspToken() {
    if (this._addonsData[this.idenediGroupId]?.length) {
      const addon = this._addonsData[this.idenediGroupId].find(
