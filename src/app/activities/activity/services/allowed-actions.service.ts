...
getActions(allowedActions: any) {
  ...
  getActionByValue(actionValue) {
    for (let action in Actions) {
      if (actionValue == Actions[action]) return action;
    }
    return null;
  }
  ...
  getAllowedActionForSignature(tab: any, flag?: any) { 
    let actions: any = [];
    if(tab == Actions.close) {
      
        actions.push({
          id: Actions.sign,
          name: this.getActionByValue(Actions.sign),
          isShow: true,
          label: "Sign",
        });
    ...
    } else if(tab == Actions.sign) {
      actions.push({
        id: Actions.revoke,
        name: this.getActionByValue(Actions.revoke),
        isShow: true,
        label: "Revoke",
      });
    //   if(!flag) {
