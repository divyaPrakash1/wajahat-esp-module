...
@Component({
  selector: "xcdrs-closed-activities",
...
export class ClosedActivitiesComponent implements OnInit {
  ...
  activeTab: number = StemeXeListType.Closed;
  isFiltersDialogOpened: boolean = false;

  allSelectedActivity: any[] = [];
  isApprovedActivity: boolean = false;
  ...
  loadUngroupedActivities(filter?: any) {
    ...
    this.isLoading = true;
    forkJoin([
      this._activitiesService.getAllClosed(
        this.search,
      ...
      this._activitiesService.getCountForClosed(
        this.search,
        this.groupByAppliedFilter,
        this.pageSize,
        this.pageIndex,
              ...
              ? resp.ResponseResult.Activities.map((ungroupedRec) => {
                  return {
                    ...
                    owner: ungroupedRec.CreatedBy
                      ...
                      : null,
                    allowedActions: ungroupedRec.IsSigned ? [] : this._allowedActions.getAllowedActionForSignature(Actions.close, ungroupedRec.IsApproved),
              ...
              this.activitiesByGroup.groups[index].groupList = !!resp
                ...
                ? resp.ResponseResult.Activities.map((listRec) => {
                    return {
                      ...
                      allowedActions: listRec.IsSigned ? [] : this._allowedActions.getAllowedActionForSignature(Actions.close, listRec.IsApproved),
  ...
  // getAllowedActions(data: any) {
  //   if (data.IsSigned)
  //     return this._allowedActions
  //       .getAllowedActionForSignature(Actions.close)
  //       .filter((x) => x.id != Actions.reject);
  //   else
  //     return this._allowedActions.getAllowedActionForSignature(Actions.close);
  // }
          ...
          if (!!resp) {
            if (resp.ResponseCode == 2000) {
              this.activitiesByGroup.weeksGroup = {
                allowedActions: this._allowedActions.getAllowedActionForSignature(
                  Actions.close
                ),
