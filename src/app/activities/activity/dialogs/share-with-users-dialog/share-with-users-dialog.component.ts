...
export class ShareWithUsersDialogComponent implements OnInit {
      ...
      if (!!resp) {
        if (resp[0].ResponseCode == 2000) {
          ...
          this.activity.dueDate =
            resp[0].ResponseResult.DueDate != null
          ...
          this.activity.creationDate =
            resp[0].ResponseResult.CreationDate != null
          ...
          this.activity.completedDate =
            resp[0].ResponseResult.CompletedDate != null
          ...
          this.activity.sharedWithUsers = !! resp[0].ResponseResult.IsShared && resp[0].ResponseResult.SharedWithUsers.length>0? resp[0].ResponseResult.SharedWithUsers.map(user=>{
            return {
              UserId:user.Id,
              UserName:user.Name,
        ...
        if (resp[1].ResponseCode === 2000) {
          this.users = resp[1].ResponseResult;
            this.users.forEach(user=>{
              user.selected =this.selectedUsers.find(sharedUser=> sharedUser.UserId == user.UserId)? true: false;
            });
  ...
  toggleItem(ev) {
    if(ev.checked){
      ...
      this.selectedUsers = this.selectedUsers.filter(user=>user.UserId !=  ev.source.value.UserId);
    }
    
    if(this.users.filter(user=> user.UserId != -2).length ===  this.selectedUsers.filter(user=> user.UserId != -2).length){
      this.isShareWithAllSelected = true;
    }else{
      this.isShareWithAllSelected = false;
    }
