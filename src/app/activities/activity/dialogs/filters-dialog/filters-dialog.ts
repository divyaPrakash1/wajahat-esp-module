...
export class FiltersDialog implements OnInit {
        ...
        (resp) => {
          if (!!resp) {
            if (resp.ResponseCode == 2000) {
              ...
              // console.log("teststetsetsetstetsetstetetse");
              // if(!this.teamId) {
  ...
  toggleAllSelection(field) {
    switch (field) {
      case "assignors":
        if (
          this.assignorsControl.value.find((val) => val.id === -2) &&
          !this.allAssignorsSelected
        ) {
          this.assignorsControl.patchValue([
            ...this.assignors.map((item) => item),
      ...
      case "jobs":
        if (
          this.jobsControl.value.find((val) => val.id === -2) &&
          !this.allJobsSelected
        ) {
          this.jobsControl.patchValue([
            ...this.jobs.map((item) => item),
      ...
      default:
        if (
          this.projectsControl.value.find((val) => val.id === -2) &&
          !this.allProjectsSelected
        ) {
          this.projectsControl.patchValue([
            ...this.teams.map((item) => item),
  ...
  toggleItem(field) {
    switch (field) {
      case "assignors":
        this.assignorsControl.patchValue(
          this.assignorsControl.value.filter((val) => val.id != -2)
        );
      ...
      case "jobs":
        this.jobsControl.patchValue(
          this.jobsControl.value.filter((val) => val.id != -2)
        );
      ...
      default:
        this.projectsControl.patchValue(
          this.projectsControl.value.filter((val) => val.id != -2)
        );
