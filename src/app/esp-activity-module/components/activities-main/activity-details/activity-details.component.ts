...
export class ActivityDetailsComponent implements OnInit {
  ...
  isEngProEnabled = false;
  isEngProDataLoaded = false;
  engProLoggedInUserId: any = null;
  isOppProEnabled = false;
  isOppProDataLoaded = false;
  oppProData: any = null;
  isEspEnabled = false;
  isEspAddonLoaded = false;
  // isEspResquestActivity: boolean = false;
  // isEspResquestActivity: boolean = false;
  espAddon!: Addon;

  isAccessedFromStacked = false;
  isAccessedFromShared = false;
  ...
  initBreadCrumb() {
    ...
    switch (this.accessedFrom) {
      ...
      case 'shared':
        ...
        this.accessedFrom == 'user'
          ? (this.innerPreTab = this._actRoute.snapshot.queryParams.innerPreTab)
          : '';
        break;
      case 'carousel':
      ...
      default:
        // list
        this.preTab = !!this._actRoute.snapshot.queryParams.preTab
          ? parseInt(this._actRoute.snapshot.queryParams.preTab)
          : null;
        this.innerPreTab = !!this._actRoute.snapshot.queryParams.innerPreTab
          ? parseInt(this._actRoute.snapshot.queryParams.innerPreTab)
          : null;
        this.isESPcomponent =
          !!this._actRoute.snapshot.queryParams.isESPcomponent &&
          this._actRoute.snapshot.queryParams.isESPcomponent == 'true'
            ? true
            : false;
        if (this.isESPcomponent) {
          this.requestName = !!this._actRoute.snapshot.queryParams.requestName
            ? this._actRoute.snapshot.queryParams.requestName
            : null;
          this.moduleName = !!this._actRoute.snapshot.queryParams.moduleName
            ? this._actRoute.snapshot.queryParams.moduleName
            : null;
          this.isMyspace = !!this._actRoute.snapshot.queryParams.my
            ? this._actRoute.snapshot.queryParams.my
            : null;
        }
        break;
    }
      ...
      if (!!data) {
        ...
        if (!!data.oppProData) {
          if (data.oppProData.code == '001' && !!data.oppProData.result) {
            ...
            this._activitiesService?.getUserInfoForStemexe(this.oppProData)?.subscribe((resp) => {
                if (!!resp) {
        ...
        if (!!resp) {
          if (resp.ResponseCode == 2000) {
            ...
            if (!!preURL && preURL != null) {
              if (
                preURL == '/pages/activities' ||
                preURL == '/pages/activities/new/assigned'
              ) {
            ...
            this.activity.appType = this.isEpmActivity
              ? 3
              : this.isEngProActivity
              ? 1
              : resp.ResponseResult.ESP_RequestId != null
              ? 4
              : 0;
            ...
            (this.activity.progressStatus = resp.ResponseResult.IsRejected
              ...
              //   // ? "Not Accepted"
              //   : "In progress";
              (this.activity.progressStatusColor = resp.ResponseResult
                .IsRejected
                ? '#EB487F'
                : resp.ResponseResult.Status == 4
                ? '#EB487F'
            ...
            this.activity.efforts = resp.ResponseResult.Efforts.map(
              (effort: any) => {
                return {
                  ...
                  endTime:
                    effort.EndTime != null
                      ? moment(effort.EndTime).format('hh:mm a')
                      : null,
                  // endTime:
                  //   effort.EndTime != null
                  //     ? moment
                  ...
                  startTime:
                    effort.StartTime != null
                      ? moment(effort.StartTime).format('hh:mm a')
                      : null,
                  // startTime:
                  //   effort.StartTime != null
                  //     ? moment
            ...
            this.activity.sharedWithUsers =
              ...
              resp.ResponseResult.SharedWithUsers.length > 0
                ? resp.ResponseResult.SharedWithUsers.map((user:any) => {
                    return {
                      UserId: user.Id,
                      UserName: user.Name,
                ...
                : [];
            this.activity.sharedWithUsersClaimed = this.activity.sharedWithUsers.filter(
              (user) => user.hasClaimed
            );
            this.activity.sharedWithUsersNotClaimed = this.activity.sharedWithUsers.filter(
              (user) => !user.hasClaimed
            );
            this.activity.isClaimedByloggedIn = resp.ResponseResult.IsShared
              ? this.getClaimStatus()
              : false;
            this.activity.isSharedByloggedIn =
              resp.ResponseResult.IsShared == true &&
              resp.ResponseResult.AssignedBy_UserId == this.loggedInUserId
                ? true
                : false;
            this.activity.requestId = resp.ResponseResult.ESP_RequestId;
            ...
            if (
              (!!this.activity.createdBy &&
                this.activity.createdBy.UserId != this.loggedInUserId &&
                !this.activity.isShared) ||
              this.activity.isShared
            ) {
              this.displayedColumns.push('assignedBy');
            }
            if (
              (!this.isEpmActivity && this.activity.parentBoardId != null) ||
              (this.isEpmActivity &&
                !!this.activity.epmInfo &&
                this.activity.epmInfo.ParentProjectId != null)
            ) {
            ...
            if (
              this.activity.appType == 4 &&
              this.activity.requestNumber != null
            ) {
            ...
            this.activity.allowedActions = this._allowedActions.getAllowedActions(
  ...
  getNames(callTo:any) {
    ...
    for (let i = 0; i < callTo.customerAttendeesList.length; i++) {
      names.push(callTo.customerAttendeesList[i].name);
    }

    for (let i = 0; i < callTo.partnerAttendeesList.length; i++) {
  ...
  getClaimStatus() {
    const loggedInuserInShareList = this.activity?.sharedWithUsersClaimed?.find(
  ...
  isMaxCharsReached(comment?: any, index?: any): any{
    // let lines =
    ...
    if (this.activity) {
      if (comment !== undefined) {
        ...
        const chars = comment != null ? comment.length : null;
