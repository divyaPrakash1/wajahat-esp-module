...
import { FileItem } from "../models/file-item";
// import { Status } from '../activities/enums';
...
export class UtilsService {
  ...
  getDueDateTimeAgo(date: string, isTacticDetail?: boolean) {
    ...
    //days less than week
      ...
      return days + (isTacticDetail ? " days Overdue" : " days ago");
    }
    //weeks
    else if (days >= 7 && days < currentMonthDaysRange) {
      let week = now.diff(dueDate, "week");
      return week <= 1 ? (isTacticDetail ? " week Overdue" : " week ago") : week + (isTacticDetail ? " weeks Overdue" : " weeks ago");
    }
    //months
    else if (days >= currentMonthDaysRange && days < 365) {
      let month = now.diff(dueDate, "month");
      return month <= 1 ? (isTacticDetail ? " month Overdue" : "Month ago") : month + (isTacticDetail ? " months Overdue" : " months ago");
    }

    //years
    else {
      let year = now.diff(dueDate, "year");
      return year <= 1 ? "Year ago" : year + " years ago";
    }
  }
  ...
  getTimeAgo(date: string) {
    ...
    var commentDate = moment(new Date(date));
